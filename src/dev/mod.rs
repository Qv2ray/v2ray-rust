...
pub(crate) async fn test_relay_ws_vmess<T: AsyncWrite + AsyncRead + Unpin>(
    ...
    let security = "auto".to_string();
    let option = VmessOption::new(&uuid, 0, &security, addr, false).unwrap();
    let ws_stream_builder =
        BinaryWsStreamBuilder::new("ws://127.0.0.1:10002/", None, vec![]).unwrap();
...
#[allow(dead_code)]
pub(crate) async fn test_direct<T: AsyncWrite + AsyncRead + Unpin>(
    t: (T, Address),
) -> io::Result<()> {
    let (inbound_stream, proxy_addr) = t;
    let outbound_stream = TcpStream::connect(proxy_addr.to_string()).await?;
    let (mut outbound_r, mut outbound_w) = tokio::io::split(outbound_stream);
    let (mut inbound_r, mut inbound_w) = tokio::io::split(inbound_stream);
    let mut down = 0u64;
    let mut up = 0u64;
    let res = tokio::try_join!(
        copy_with_capacity_and_counter(
            &mut outbound_r,
            &mut inbound_w,
            &mut down,
            LW_BUFFER_SIZE * 4
