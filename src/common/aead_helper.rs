...
use aes_gcm::{AeadInPlace, Aes128Gcm, Aes256Gcm, Key as GcmKey, Nonce as GcmNonce, aead::Tag, AeadCore};
...
pub trait AeadCipherHelper:AeadInPlace{
    ...
    fn encrypt_inplace_with_slice(&self,nonce:&[u8],aad:&[u8],mut buffer:&mut [u8]){
        ...
        let x=self.encrypt_in_place_detached(nonce, aad,msg).expect("encryption failure!");
    ...
    fn decrypt_inplace_with_slice(&self,nonce:&[u8],aad:&[u8],buffer:&mut [u8])->bool{
        ...
        let tag_pos = buffer.len() - Self::TagSize::to_usize();
        let (msg, tag) = buffer.as_mut().split_at_mut(tag_pos);
        self.decrypt_in_place_detached(nonce, aad, msg, Tag::<Self>::from_slice(tag)).is_ok()
    }
}
