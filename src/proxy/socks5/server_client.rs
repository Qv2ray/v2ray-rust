...
impl ServerClient {
    ...
    pub async fn init(&self, stream: &mut TcpStream) -> io::Result<()> {
        ...
        match buf[1] {
            socks_command::CONNECT => {
                address = Address::read_from_stream(stream).await?;
            ...
            socks_command::UDP_ASSOSIATE => {
                address = Address::read_from_stream(stream).await?;
                let socket_addr = SocketAddr::new(stream.local_addr().unwrap().ip(), 0);
                let udp_socket = UdpSocket::bind(socket_addr).await?;
                let addr = Address::SocketAddress(socket_addr);
